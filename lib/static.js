...
if (await checkExists(configOldDir) && !await checkExists(configDir)) {
    console.warn(style.yellow(`Config directory ${configOldDir} is deprecated.\nMoving to ${configDir}.\n`));
    try {
        await fs.rename(configOldDir, configDir);
    } catch (e) {
        console.error(style.red(`✗ Failed to move config directory: ${e.message}`));
    }
}
...
export async function getCssDoodleLib() {
    ...
    console.warn(
        style.yellow(`css-doodle in config not found: ${libFile}.\nUsed default css-doodle instead.\n`),
    );
...
export async function getBrowserPath() {
    ...
    if (browserPath) {
        if (await checkExists(browserPath)) {
            ...
            console.warn(style.yellow(`Browser in config not found: ${browserPath}.\nUsed default browser instead.\n`));
        }
    }
    return '';
}
...
async function getConfigContent(configFilePath) {
    try {
        ...
        console.error(style.red(`✗ Failed to access config file: ${e.message}`));
        return {};
    }
}
