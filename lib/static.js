    ...
    if (libPath) {
        if (fs.existsSync(libPath)) {
            ...
            console.warn(msgWarning(`
                css-doodle in config not found: ${libPath}.
                Used default css-doodle instead. Please check it with '${msgCommand("config")}' command.
            `));
            console.log();
        }
    }
...
export function getBrowserPath() {
    ...
    if (browserPath) {
        if (fs.existsSync(browserPath)) {
            ...
            console.warn(msgWarning(`
                browser not found: ${browserPath}.
                Used default browser instead. Please check it with '${msgCommand("config")}' command.
            `));
            console.log();
        }
    }
