...
export async function render(code, options = {}) {
    ...
    const output = options.time ? await screencast(page, options) : await screenshot(page, options);
...
function getOutputInfo(options = {}) {
    ...
    if (time) {
        if (format && !supportedOutputFormat.video.includes(format)) {
            throw new Error(
                `time option is only supported for video formats: ${supportedOutputFormat.video.join(', ')}`,
            );
        }
        if (!format) {
