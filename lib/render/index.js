...
import { clearRow, log, question, style } from '../utils.js';
...
export async function render(code, options = {}) {
    ...
    try {
        ...
        const proceed = await processIfOutputExists(options.output, options.yes, options.quiet);
        if (!proceed) {
            log.warn('aborted');
            return;
        }
...
async function processIfOutputExists(output, yes, quiet = false) {
    try {
        await fs.access(output, fs.constants.F_OK);
        if (yes) {
            return true;
        }
        // In non-TTY environments (e.g., CI), can't prompt - fail gracefully
        if (!process.stdout.isTTY) {
            if (!quiet) {
                log.error(`"${output}" already exists. Use -y/--yes to overwrite or choose a different output name.`);
            }
            return false;
        }
