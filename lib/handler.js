...
export async function handleRender(source, options) {
    ...
    if (options.window) {
        ...
        if (!Number.isFinite(width) || width <= 0 || !Number.isFinite(height) || height <= 0) {
            log.error(`invalid window size '${options.window}', expected format: WIDTHxHEIGHT (e.g., 1600x1000)`);
            process.exit(1);
        }
        if (width > 8192 || height > 8192) {
            log.error(`window size too large, maximum is 8192x8192`);
            process.exit(1);
        }
    ...
    if (options.scale) {
        ...
        if (!Number.isFinite(scale) || scale <= 0 || scale > 10) {
            log.error(`invalid scale '${options.scale}', expected a number between 0.01 and 10`);
            process.exit(1);
        }
        options.scale = scale;
    }
...
export async function handlePreview(source, options) {
    const { content, error, type } = await read(source);
    if (error) {
        const isEmptyError = error.message?.includes('empty') ?? false;
        if (!(type === 'css' && isEmptyError)) {
            log.error(error.message);
            process.exit(1);
        }
    }
