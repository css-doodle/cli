...
export async function handlePreview(source, options) {
    ...
    if (type === 'codepen' || type === 'html' || type === 'webpage') {
        return log.error('only .css and .cssd files are supported for preview');
    }
    if (source === undefined) {
    ...
    if (field === 'css-doodle') {
        try {
            ...
            if (await isValidCssDoodleFile(value)) {
                ...
                return log.error(`invalid css-doodle package file '${value}'`);
            }
        } catch (_e) {
            if (!isPackageVersion(value)) {
                return log.error(`invalid package version '${value}'`);
            }
