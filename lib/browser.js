...
export async function resolveBrowser(quiet = false) {
    ...
    if (systemBrowser) {
        if (!quiet) {
            console.log(style.dim(`using system browser: ${systemBrowser}`));
        }
    ...
    // 3. Download Chrome to standard Puppeteer cache
    if (!quiet) {
        console.log(style.yellow('no system browser found, downloading Chrome...'));
    }
...
async function downloadBrowser(quiet = false) {
    ...
    try {
        ...
        if (!quiet) {
            console.log(style.green(`downloaded Chrome: ${installed.executablePath}`));
        }
